...
import net.nightwhistler.pageturner.activity.PDFReaderActivity;
   ...
   // ViewGroup altLinkParent;
    ...
    public void startDownload(final boolean openOnCompletion, final String url) {
        ...
        if ( !openOnCompletion && Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH ) {
            task.setCallBack( new NotificationBarCallback(getActivity().getBaseContext(), title, openOnCompletion), title);
        } else {
            task.setCallBack( new ProgressDialogCallback(getActivity().getBaseContext(), task, openOnCompletion),title );
        }
    ...
    private abstract class AbstractDownloadCallback implements DownloadFileTask.DownloadFileCallback {
        ...
        protected Intent getBookOpenIntent(File destFile) {
            ...
            if(destFile.getName().endsWith(".pdf")){
                ...
                intent.putExtra("PDF",Uri.parse(destFile.getAbsolutePath()).toString());
