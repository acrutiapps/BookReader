...
public class LibraryFragment extends RoboSherlockFragment implements ImportCallback, OnItemClickListener,
	...
	private void openBook(LibraryBook libraryBook) {
        ...
        if(libraryBook.getFileName().endsWith(".pdf")){
            intent = new Intent(getActivity(), PDFReaderActivity.class);
            LOG.debug(Uri.parse(libraryBook.getFileName()).toString());

            intent.putExtra("PDF",Uri.parse(libraryBook.getFileName()).toString() );
